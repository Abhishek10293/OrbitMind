...
function SearchAssistant({ list }: { list: AssistantType[] }) {
  ...
  const { setAssistant } = useContext(AssistantContext); // removed unused 'assistant'
  ...
  return (
    <div className='relative'>
      ...
      {showDropDown && searchTerm && (
        <div className='absolute top-full mt-1 w-full bg-white dark:bg-black border border-gray-200 dark:border-slate-800 rounded-lg shadow-md z-50 max-h-60 overflow-y-auto'>
          {list
            .filter((assist) =>
